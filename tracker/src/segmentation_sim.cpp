...
class segmentation_sim {

	public:

		explicit segmentation_sim(ros::NodeHandle nh) : m_nh(nh)  {

		// define the subscriber and publisher
		m_sub = m_nh.subscribe ("/camera/depth_registered/points",1, &segmentation_sim::cloud_cb, this);
...
//Aucone-Bagheri
void segmentation_sim::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
...
*/
	...
	for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
	{
		...
		// rotate the cluster
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr trasformedCluster (new pcl::PointCloud<pcl::PointXYZRGB>()); 
		pcl::transformPointCloud(*clusterPtr, *trasformedCluster, transformation);

		// Compute principal directions
		Eigen::Vector4f pcaCentroid;
		pcl::compute3DCentroid(*trasformedCluster, pcaCentroid);
		Eigen::Matrix3f covariance;
		computeCovarianceMatrixNormalized(*trasformedCluster, pcaCentroid, covariance);
		...
		pcl::transformPointCloud(*trasformedCluster, *cloudPointsProjected, projectionTransform);
		...
		*/
		...
		pcl::toPCLPointCloud2(*trasformedCluster, outputPCL);
...
{
	// Initialize ROS
	ros::init (argc, argv, "segmentation_sim");
	ros::NodeHandle nh;

	segmentation_sim segs(nh);
	ros::Rate loop(10);
