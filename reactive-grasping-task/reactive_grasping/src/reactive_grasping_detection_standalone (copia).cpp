...
ReactiveGraspingDetection::ReactiveGraspingDetection() {
  ...
  // std::vector<double> a = {1, -2.8998, 2.8045, -0.9046};
  // std::vector<double> b = {0.0004670, -0.0004066, -0.0004066, 0.0004670}; 
  //   A0 = 1; eb param
  ...
  //pezzo pre ga-cds
  // std::vector<double> a = {1, -0.98441412741609691};
  // std::vector<double> b = {0.0077929362919515527, 0.0077929362919515527};
  //pezzo pre ga-cds
  ...
  private_node_handle_->param("contact_direction_topic_name_", contact_direction_topic_name_, std::string(DEFAULT_CONTACT_DIRECTION_TOPIC_NAME));
  ...
  //temp_file_.open("/home/manuelb/Desktop/temp_file_.txt");
  ...
  //SIMONE CIOTTI - START
  file_simone_1.open("/home/manuelb/Desktop/dati_filt_and_norm_simone.txt");
  file_simone_2.open("/home/manuelb/Desktop/dati_raw_simone.txt");
  file_simone_3.open("/home/manuelb/Desktop/dati_filt_simone.txt");
  //SIMONE CIOTTI - END
  ...
  contact_direction_publisher_ = node_handle_.advertise<std_msgs::String>(contact_direction_topic_name_, topic_queue_length_);
...
int ReactiveGraspingDetection::checkOscillation(int imu_id) {
  ...
  //if ((values_x[0] > contact_threshold_ /3 && (values_x[0]*values_x[1] > 0 && values_x[0]*values_x[2] > 0 && values_x[0]*values_x[3] > 0 && values_x[0]*values_x[4] > 0 && values_x[0]*values_x[5] > 0 && values_x[0]*values_x[6] > 0 && values_x[0]*values_x[7] > 0 && values_x[0]*values_x[8] > 0 && values_x[0]*values_x[9] > 0 && values_x[0]*values_x[10] > 0 && values_x[0]*values_x[11] > 0 ))
  //    || (values_y[0] > contact_threshold_ /3 && (values_y[0]*values_y[1] > 0 && values_y[0]*values_y[2] > 0 && values_y[0]*values_y[3] > 0 && values_y[0]*values_y[4] > 0 && values_y[0]*values_y[5] > 0 && values_y[0]*values_y[6] > 0 && values_y[0]*values_y[7] > 0 && values_y[0]*values_y[8] > 0 && values_y[0]*values_y[9] > 0 && values_y[0]*values_y[10] > 0 && values_y[0]*values_y[11] > 0))
  //    || (values_z[0] > contact_threshold_ /3 && (values_z[0]*values_z[1] > 0 && values_z[0]*values_z[2] > 0 && values_z[0]*values_z[3] > 0 && values_z[0]*values_z[4] > 0 && values_z[0]*values_z[5] > 0 && values_z[0]*values_z[6] > 0 && values_z[0]*values_z[7] > 0 && values_z[0]*values_z[8] > 0 && values_z[0]*values_z[9] > 0 && values_z[0]*values_z[10] > 0 && values_z[0]*values_z[11] > 0))) 
  //{
  //  return 1;
  //}

  // if ((values_x[0] > contact_threshold_ /3 && (values_x[0]*values_x[1] > 0 && values_x[0]*values_x[2] > 0 && values_x[0]*values_x[3] > 0 && values_x[0]*values_x[4] > 0 && values_x[0]*values_x[5] > 0 && values_x[0]*values_x[6] > 0 && values_x[0]*values_x[7] > 0 ))
  //     || (values_y[0] > contact_threshold_ /3 && (values_y[0]*values_y[1] > 0 && values_y[0]*values_y[2] > 0 && values_y[0]*values_y[3] > 0 && values_y[0]*values_y[4] > 0 && values_y[0]*values_y[5] > 0 && values_y[0]*values_y[6] > 0 && values_y[0]*values_y[7] > 0 ))
  //     || (values_z[0] > contact_threshold_ /3 && (values_z[0]*values_z[1] > 0 && values_z[0]*values_z[2] > 0 && values_z[0]*values_z[3] > 0 && values_z[0]*values_z[4] > 0 && values_z[0]*values_z[5] > 0 && values_z[0]*values_z[6] > 0 && values_z[0]*values_z[7] > 0 ))) 
  // {
  //   return 1;
  // }
...
int ReactiveGraspingDetection::detectContact() {
  ...
  //SIMONE CIOTTI - START
  for(auto const &val : accel_abs_values)
  {
    file_simone_1 << val << " ";
  }
  file_simone_1 << std::endl;
  file_simone_1.flush();
  //SIMONE CIOTTI - END
  ...
  if (*it_accel_max > contact_threshold_) {
    ...
    if (accel_filt_.at(imu_id).abs_contribution.at(window_size_/3 + 1) > contact_threshold_
      ...
      if(imu_id == 0)
      {
        if((accel_filt_.at(imu_id).abs_contribution.at(window_size_/3 + 1) < 200
          && accel_filt_.at(imu_id).abs_contribution.at(window_size_/3 + 2) < 200))
          {
            return -1;
          }
          else
      ...
      // if (checkOscillation(imu_id) != 1) {
        //contact_detected_ = true;
        // false_positive_ = true;
        //skip_samples_ = 3*window_size_;
        // ROS_INFO_STREAM("Attento qui c'è un falso positivo. P.S. colpa di Franco");
      // }
      // else{ 
        false_positive_ = false;  
        num_contacts_detected_++;
        // ROS_INFO_STREAM("Questo per me è buono. Bravo Franco");

      // }  
         //ROS_INFO_STREAM("contact threshold:" <<contact_threshold_);
        //ROS_INFO_STREAM("We are here...");
      if (!hand_closed_) {
        if(first_print)
        {
          ROS_INFO_STREAM("[Detection::detectContact] IMU: " << imu_id 
                           << ", acc contact value:" << *it_accel_max);
                           //accel_raw_.at(imu_id).y.at(window_size_/3));
          first_print = false;
        }
...
void ReactiveGraspingDetection::filterAccelerations() {
  ...
  for (auto &imu : accel_filt_) {
    ...
    file_simone_2 << (*it_raw).x.back() << " " << (*it_raw).y.back() << " " << (*it_raw).z.back() << " ";
    file_simone_3 << imu.x.back() << " " << imu.y.back() << " " << imu.z.back() << " ";
    ...
    // std::vector<double> vi_output;

    // vi_output = filter(filter_coeff_b_, filter_coeff_a_, vi, vi_output);
    // //temp_file_ << vi_output << "\n";
    // for (int ind = 0; ind < 100; ind++){
    //   temp_file_ << vi_output[ind] << "\t";
    // } 
    // temp_file_ << std::endl;// << "\n";
...
void ReactiveGraspingDetection::processData(std::vector<reactive_grasping::GloveIMU> data,
  ...
  //ROS_INFO_STREAM("We are here 528");
  ...
  //ROS_INFO_STREAM("We are here 541");
  ...
  // if (!contact_detected_) {
    ...
    if (imu_id >= 0 && !false_positive_) {    
      ...
      std_msgs::String contact_direction_msg;
      contact_direction_msg.data = approaching_direction;
    
      contact_direction_publisher_.publish(contact_direction_msg);
      ...
      if (approaching_direction == "fake_finger") {only_detection_ = 1;}
      //ROS_INFO_STREAM("only_dection " << only_detection_);
      //ROS_INFO_STREAM("calibration " << calibration_);
